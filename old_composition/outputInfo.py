...
def outputInformation(results_dir, operator, TypeId, timeout, cardP, cardBest, divT, total_time, all_ppc_time, all_ppc_num, all_cross_time, all_cross_num):
    ...
    if all_ppc_num != 0:
        ...
        print("Consistency checking time in average: %f. (total time: %f, number: %d)" % (ppc_ave, all_ppc_time, all_ppc_num), file=f)
    if all_cross_num != 0:
    ...
    if globs["fcomp_num"] != 0:
        ...
        print("Compositions pre-calculation time in average: %f. (total time: %d, number: %d)" % (fcomp_ave, globs["fcomp_total"], globs["fcomp_num"]), file=f)
